...
function cardClick(cat) {
  const resumeImg = document.getElementById('resumeImg');
  resumeImg.setAttribute('src', cat.img_link);
  document.getElementById('resumeName').innerText = cat.name;
  document.getElementById('resumeAge').innerText = cat.age;
  document.getElementById('resumeText').innerText = cat.description;
...
function addElementToPage({ name, img_link, age, rate, favourite, description, id }, container) {
  ...
  p.innerText = name;
  catRate.classList.add('card__rate');
  addCatRate(rate, catRate);

  div.onclick = cardClick.bind({}, { name, age, description, img_link });
  div.appendChild(img);
  div.appendChild(p);
  div.appendChild(catRate);
  div.classList.add("card");
  container[0].appendChild(div);
};

function addCatRate(rate, container) {
  for (let i = 0; i < 10; i++) {
    let src;
    let smile;
    let imgRate = document.createElement('img');
    if (i < rate) {
      src = 'img/cat-fill.svg';
      smile = '^_^';
    } else {
      src = 'img/cat-stroke.svg';
      smile = 'o_O';
    }
    imgRate.setAttribute('src', src);
    imgRate.setAttribute('alt', smile);
    container.appendChild(imgRate);
  }
}
...
window.onload = (function() {
  ...
  cats.forEach(cat => {
    addElementToPage(cat, container);
  });
})();